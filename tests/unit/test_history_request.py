...
def test_request_derives_ids_evidence_hashes_and_staging_budget(tmp_path: Path) -> None:
    registry, capacity = evidence_files(tmp_path)
    request = request_payload()
    schema = json.loads(
        (ROOT / "schemas" / "market" / "v1" / "bybit-1m-history-request.schema.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    assert tuple((item.instrument_id, item.symbol) for item in resolved.spec.series) == (
        (1, "AAAUSDT"),
        (9, "BBBUSDT"),
    )
    ...
    assert resolved.budget.rest_staging_bytes == (
        STAGING_METADATA_BYTES + 2 * MAX_PAGE_ARTIFACT_BYTES
    )


def test_request_cannot_supply_identity_or_use_wrong_layout(tmp_path: Path) -> None:
    registry, capacity = evidence_files(tmp_path)
    raw = request_payload()
    raw["series"][0]["instrument_id"] = 123  # type: ignore[index]
    with pytest.raises(HistoryAcquisitionError, match="series fields"):
        resolve_history_request(
    ...
    other_root.mkdir()
    registry, capacity = evidence_files(other_root, bucket_count=4)
    with pytest.raises(HistoryAcquisitionError, match="accepted canonical layout"):
        resolve_history_request(
            write_request(other_root, request_payload()),
            instrument_registry_path=registry,
            capacity_evidence_path=capacity,
        )


def test_request_rejects_prelaunch_range_and_cross_bucket_job(tmp_path: Path) -> None:
    registry, capacity = evidence_files(tmp_path)
    before_launch = request_payload(
        series=[
            {
                "symbol": "AAAUSDT",
    ...
    with pytest.raises(HistoryAcquisitionError, match="precedes registry launch"):
        resolve_history_request(
            write_request(tmp_path, before_launch),
            instrument_registry_path=registry,
            capacity_evidence_path=capacity,
        )
    ...
    with pytest.raises(HistoryAcquisitionError, match=r"unique|month/bucket"):
        resolve_history_request(
...
def test_closed_before_is_current_minute_boundary() -> None:
    assert closed_before_now_ms(JANUARY_1_2026_MS + 59_999) == JANUARY_1_2026_MS
    with pytest.raises(HistoryAcquisitionError, match="current time"):
        closed_before_now_ms(True)  # type: ignore[arg-type]
